 ...
 * 1. 单个空闲链表都按照 size 大小升序排列，使用 first_fit ，实现了 best_fit 的效果
 *
 * 编码：
 * 1. 按照功能拆分了一些函数
 * 1. 使用了两个全局变量，一个保存空闲链表的入口地址，另一个保存序言块的起始地址（这个好像也不太需要。。）
 * 1. 涉及到指针的特殊处理，基本都使用了宏定义
 * 1. 函数头，对函数的注意事项进行了必要的描述
 *
 * 活动：
 * 20201001 -- 阅读 csapp
 * 20201003 -- 基本阅读完毕，在网上搜了一下，感觉都是乱七八糟的（评论中基本都有说运行段错误），使用 google 也没有搜索到好的实现，决定开始自己写；确定空闲链表使用双向指针
 * 20201004 -- 开始书写，开始一直感觉无法集中注意力，总是想去做其他的事情
 * 20201005 -- 功能函数基本完成
 * 20201006 -- 开始调试，，增加了堆的检查函数，居然检查函数写错了好几个地方，后来发现最小块大小有问题，24 的时候，在空闲链表上无法正常保存，晚上可以跑通，但是分数很低
 * 20201007 -- 增加了上述注释信息，提交 github 保存；耗费了 7 天的时间，外加至少 1000 大洋的加班费。昨天跑分好像是 69 ，今天不清楚为什么变成了 54 ，利用率有两个很低，可能和 realloc 函数没有实习有关系，另外效率好像也很低，难道不应该释放后立即合并？ TODO
 */
